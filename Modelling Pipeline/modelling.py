...
# Treatment definition: agreement must contain an explicit Health provision.
# IPR-only agreements (~50% of all PTAs) are excluded — they govern pharmaceutical
# patents but carry no direct health access or coverage mandate.
...
#
#  Bloc 1 — EU/EC (27 countries, always-treated since 1958):
#    Reclassified as gname=0 clean controls. High-income, stable trajectories,
#    near-ceiling immunization coverage. Comparable to USA/CAN/AUS in treated group.
#
#  Bloc 2 — Staggered treated (11 countries):
#    Countries whose first Health-provision PTA entered into force 2001-2023.
#    Kept as-is; country-level cleaning later drops those without ≥3 pre-years.
#
#  Bloc 3 — Never-treated controls (explicit whitelist):
#    High-income non-EU  : GCC states, EFTA, GBR, ISR, NZL, PAN, URY
#    Upper-middle-income : Eastern Europe, Latin America, SE Asia, Central Asia, MENA
#    All other never-treated countries (CARIFORUM small-island states, Sub-Saharan
#    Africa, very-low income) are dropped — too structurally dissimilar.
EU_BLOC = {
...
NEVER_TREATED_KEEP = {
    # High-income non-EU
    "BHR","KWT","OMN","QAT","SAU","ARE",   # GCC
    "CHE","ISL","NOR",                      # EFTA
    "GBR","ISR","NZL","PAN","URY",          # other HI
    # Upper-middle-income (structural comparators for treated group)
    "ALB","ARM","AZE","BIH","BLR","MKD","MNE","SRB",  # Eastern Europe
    "ARG","BOL","BRA","ECU","PER","PRY",               # Latin America
    "IDN","THA","LKA",                                 # Southeast Asia
    "KAZ",                                             # Central Asia
    "DZA","LBN","TUN",                                 # MENA
}

# Drop non-EU always-treated; drop never-treated not in the explicit whitelist
df = df[
    ~((df['treatment_group'] == 'always_treated') & ~df['country_iso3'].isin(EU_BLOC)) &
    ~((df['treatment_group'] == 'never_treated')  & ~df['country_iso3'].isin(NEVER_TREATED_KEEP))
].copy()

# Reclassify EU always-treated → never_treated so they enter as gname=0 controls
df.loc[
    (df['treatment_group'] == 'always_treated') & df['country_iso3'].isin(EU_BLOC),
    'treatment_group'
] = 'never_treated'

print('\nAfter control group construction:')
print(df.groupby('treatment_group')['country_iso3'].nunique())
...
# EU countries were reclassified to never_treated but still carry their historical
# PTA start year (1958) from the WTO-X merge — override to 0 so they enter the
# DiD as clean controls alongside the never-treated group.
panel.loc[panel['country_iso3'].isin(EU_BLOC), 'gname'] = 0

# ── Window restriction — country-level cleaning ────────────────────────────────
# Drop individual countries that cannot satisfy the pre-window requirement rather
# than dropping entire cohorts.  A cohort survives if at least one country in it
# has sufficient pre-data; countries that don't are removed from that cohort only.
# This recovers JOR (cohort 2005) and MYS (cohort 2011) which would otherwise be
# lost because a cohort-mate (SGP / JPN) starts data at or after treatment.
_DESIRED_PRE  = -3   # 3 pre-treatment years — credible for parallel trends testing
_DESIRED_POST = 10
_MIN_COHORT_SIZE = 1  # keep singleton cohorts (each Health-PTA country is unique)

_treated = panel[panel['gname'] > 0].copy()

# Per (cohort, country): earliest year in data + whether gname-1 (LP-DiD base) exists
_cc = (
    _treated
    .assign(_is_base=lambda d: d['year'] == d['gname'] - 1)
    .groupby(['gname', 'country_iso3'])
    .agg(min_year=('year', 'min'), has_base=('_is_base', 'any'))
    .reset_index()
)
_cc['req_year'] = _cc['gname'] + _DESIRED_PRE
_cc['is_clean'] = (_cc['min_year'] <= _cc['req_year']) & _cc['has_base']

# Report dropped countries
_drop_ctry = _cc[~_cc['is_clean']]
if not _drop_ctry.empty:
    print(f'\nDropping {len(_drop_ctry)} country-cohort pairs (insufficient pre-data or no base period):')
    print(_drop_ctry[['gname','country_iso3','min_year','req_year','has_base']].to_string(index=False))

# Cohorts with enough clean countries
_cohort_n      = _cc[_cc['is_clean']].groupby('gname').size()
_valid_cohorts = _cohort_n[_cohort_n >= _MIN_COHORT_SIZE].index.tolist()
_thin_cohorts  = _cohort_n[_cohort_n <  _MIN_COHORT_SIZE].index.tolist()
if _thin_cohorts:
    print(f'\nDropping empty cohorts: {sorted(_thin_cohorts)}')

print(f'\nValid cohorts: {sorted(_valid_cohorts)}')
for _g in sorted(_valid_cohorts):
    _countries = _cc.loc[_cc['is_clean'] & (_cc['gname'] == _g), 'country_iso3'].tolist()
    print(f'  gname={_g}: {_countries}')

# Rebuild treated panel: only clean country-cohort pairs in valid cohorts
_clean_keys    = _cc.loc[_cc['is_clean'] & _cc['gname'].isin(_valid_cohorts),
                          ['gname', 'country_iso3']]
_treated_clean = _treated.merge(_clean_keys, on=['gname', 'country_iso3'], how='inner')
panel = pd.concat([panel[panel['gname'] == 0], _treated_clean], ignore_index=True).copy()

# All remaining treated units satisfy _DESIRED_PRE by construction
_PRE = _DESIRED_PRE
...
panel = panel[
    (panel['gname'] == 0) |
    ((_rel >= _PRE) & (_rel <= _DESIRED_POST))
].copy()

# Cap post-window at actual max in filtered data (lpdid validates against observed values)
_actual_post = int(
    (panel.loc[panel['gname'] > 0, 'year'] - panel.loc[panel['gname'] > 0, 'gname']).max()
)
_POST = min(_actual_post, _DESIRED_POST)
print(f'\nPre-window  _PRE : {_PRE}')
print(f'Post-window _POST: {_POST}  (data max: {_actual_post})')
